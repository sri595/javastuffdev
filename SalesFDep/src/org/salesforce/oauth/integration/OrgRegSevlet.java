 ...
 * Servlet parameters
 */
@WebServlet(name = "OrgRegSevlet", urlPatterns = { "/OrgRegSevlet/*",
		"/OrgRegSevlet" }, initParams = {
		...
		@WebInitParam(name = "redirectUri", value = "https://localhost:8443/SalesFDep/OAuthServlet/callback"),
		@WebInitParam(name = "environment", value = "https://login.salesforce.com"), })
public class OrgRegSevlet extends HttpServlet {
	private static final long serialVersionUID = 1L;

	private static final Logger LOG = LoggerFactory
			.getLogger(OrgRegSevlet.class);
	...
	protected void doGet(HttpServletRequest request,
		...
		if (request.getRequestURI().endsWith("OrgRegSevlet")) {
		...
		try {
			...
			if (get.getStatusCode() == HttpStatus.SC_OK) {
				...
				try {
					...
					Cache.getMap().put(jsonResponse.get("organization_id"),
							accessToken);
				} catch (JSONException e) {
					LOG.error(
