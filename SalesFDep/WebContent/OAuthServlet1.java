 ...
 * Servlet parameters
 */
@WebServlet(name = "OAuthServlet1", urlPatterns = { "/OAuthServlet1/*",
		...
		@WebInitParam(name = "redirectUri", value = "https://183.82.108.79/SalesFDep/OAuthServlet/callback"),
...
public class OAuthServlet1 extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request,
		...
		if (request.getRequestURI().endsWith("OAuthServlet")) {
			System.out.println("Request URL corect : " + request.getRequestURI());
			if ((request.getParameter("code") == null || request.getParameter(
					"code").isEmpty())) {
				// Send the user to authorize
				System.out.println("Redirecting to : " + authUrl + "--code--"
						+ request.getParameter("code"));
				response.sendRedirect(authUrl);
				return;
			}
			else {
				System.out.println("Where am i : " + request.getRequestURI());
				String code = request.getParameter("code");
				System.out
						.println("Auth successful, got Authorization code: {} "
								+ code);
				...
				try {
					System.out
							.println("Auth successful, post.getURI().toString() "
					...
					try {
						...
						System.out
								.println("Auth successful, authResponse.toString(2) "
										+ post.getURI().toString());
						LOG.info("Auth Response: {} ", authResponse.toString(2));
					} catch (JSONException e) {
						System.out.println("Auth successful, exception "
								+ e.getMessage());
						LOG.error(
					...
					System.out.println("Auth successful, exception1 "
							+ e.getMessage());
				...
				// response.sendRedirect(request.getContextPath() + "/sfhome");
				response.sendRedirect("https://na16.salesforce.com/home/home.jsp");
			}
		}
